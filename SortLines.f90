subroutine SortLines()
  ...
  implicit none
  real(kind=8) :: lam_min
  integer :: i, imin, imolmin, maxlines, imol
  logical :: found
  ...
  do imol = 1, nmol
    ...
    if (Mol(imol)%nlines > maxlines) maxlines = Mol(imol)%nlines
  enddo
  ...
  do while (found)
    ...
    do imol = 1, nmol
      do i = 1, Mol(imol)%nlines
        if (Mol(imol)%L(i)%lam <= lam_min .and. &
            Mol(imol)%L(i)%lam > lmin .and. .not. used(imol,i)) then
