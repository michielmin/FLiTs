subroutine RaytraceLines()
  ...
  real(kind=8) :: lam, T, Planck, wl1, wl2, v, flux0, starttime, stoptime, tot, fact, lcmin
  ...
  real(kind=8) :: wl11, wl21, wl12, wl22, wl13, wl23, flux_l1, flux_l2, flux_c
...
! DOC by Claude
  ...
  real(kind=8) :: tau, exptau, flux, fact, prof, S, tau_gas, tau_dust, tau_d, tau_tot
  type(Path) :: p0
...
subroutine TraceFluxLines(p0, flux, ii, vmult, imol_blend, v_blend, doit, nb, ib0)
  ...
  real(kind=8) :: tau, exptau, flux, fact, prof, S, tau_gas, tau_dust, tau_d, tau_tot, v_blend(nb)
  type(Path) :: p0
  type(Cell), pointer :: CC
  type(Blend) :: Bl
  logical :: gas, doit(nb)
...
subroutine Trace2StarLines(p0, flux, ii, imol_blend, v_blend, nb)
  ...
  real(kind=8) :: tau, flux, prof, v_blend(nb)
  type(Path) :: p0
  type(Cell), pointer :: CC
  type(Blend) :: Bl
...
subroutine InterpolateLam(lam0, ilam)
  ...
  use Constants
  implicit none
  real(kind=8) :: wl1, wl2, w1, w2, lam0, x1, x2
  integer :: ilam, i, j
